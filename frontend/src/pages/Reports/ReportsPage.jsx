  ...
  const handleCreateReport = async () => {
    try {
      ...
      notify({
        ...
        message: "New PDF report compiled successfully.",
      });
    } catch {
      notify({
        type: "error",
        title: "Compilation Failed",
        message: "Unable to compile the report. Please try again.",
      });
    }
  };
    ...
    try {
      ...
      if (report?.viewUrl) {
        ...
        notify({
          type: "error",
          title: "View Failed",
          message: "This report has no viewable PDF yet.",
        });
      }
    } catch {
      notify({
        type: "error",
        title: "View Failed",
        message: "Unable to open the report. Please try again.",
      });
    }
  };
  ...
  const handleDownloadReport = async (reportId) => {
    try {
      ...
      notify({
        ...
        message: `${report.filename || reportId}.pdf downloaded.`,
      });
    } catch {
      notify({
        type: "error",
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-3 bg-[#050816]/60 border border-accent/5 rounded-lg">
                    ...
                    <div className="flex items-center justify-between gap-3 py-1 font-mono text-[9px] text-slate-400">
                      <span className="leading-none">
                        Include raw OpenCV coordinates:
                      </span>
                      <input
                        type="checkbox"
                ...
                <Button
                  variant="primary"
                  className="!mt-5 !h-[50px] w-full flex items-center justify-center gap-2"
            ...
            {loading ? (
              ...
              displayReports.map((batch) => (
                ...
                >
                  ...
                  {/* Report Title */}
                  <h3 className="mt-2.5 font-display text-xs uppercase tracking-wider text-white font-bold">
                    {batch.title}
                  </h3>
                    ...
                    <span>
                      {batch.pcbId ? (
                        <span className="text-slate-400">
                          PCB: <strong>{batch.pcbId}</strong>
                        </span>
                      ) : null}
                    </span>
                    <span>
                      Size:{" "}
                      <strong className="text-slate-400">{batch.size}</strong>
                    </span>
                  </div>
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3 border-t border-accent/5 pt-2.5">
                    <span
                      className={`flex items-center gap-1 font-mono text-[8px] tracking-wider px-2 py-1 border rounded uppercase font-bold ${
